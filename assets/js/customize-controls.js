/* global RadioProBgColors, RadioProColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( RadioProBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( RadioProBgColors[ context ].color ) {
						backgroundColorValue = RadioProBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( RadioProBgColors[ context ].setting ).get();
					}
					RadioProSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( RadioProBgColors ).forEach( function( context ) {
			wp.customize( RadioProBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					RadioProSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		RadioProSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				RadioProSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function RadioProSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = RadioProColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function RadioProSetRetineLogoVisibility( visible ) {

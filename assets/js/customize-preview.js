/* global RadioProBgColors, RadioProPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( RadioProBgColors ).forEach( function( context ) {
					RadioProGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function RadioProGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'RadioPro-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#RadioPro-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( RadioProPreviewEls[ context ] ) {
				_.each( RadioProPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		RadioProGenerateColorA11yPreviewStyles( 'content' );
		RadioProGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
